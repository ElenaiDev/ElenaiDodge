...
public class DisableAirborne implements ICommand {

	private final List<String> aliases;

	public DisableAirborne() {
		aliases = new ArrayList<String>();
		aliases.add("disable_airborne");
	...
	public String getName() {
		return "disable_airborne";
	}

	@Override
	public String getUsage(ICommandSender sender) {
		return "/elenaidodge disable_airborne <player>";
	}
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		if (args.length == 0) {
			sender.sendMessage(new TextComponentString(TextFormatting.RED + "/elenaidodge disable_airborne <player>"));
		} else
			try {
				...
				if (!world.isRemote) {
					...
					sender.sendMessage(new TextComponentString(
							TextFormatting.RED + "Disabled dodging whilst airborne for " + target.getName() + "!"));
				}
